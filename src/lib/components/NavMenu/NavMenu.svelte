<script lang="ts">
	...
	const items = [
		{ title: NavBar.skills, to: '/skills', icon: 'i-carbon-webhook' },
		{ title: NavBar.personal, to: '/projects', icon: 'i-carbon-chemistry' },
		{ title: NavBar.career, to: '/experience', icon: 'i-carbon-network-4' },
		...
		<div class="flex flex-row flex-1 self-center justify-center">
			{#each items as item}
				<a
					...
					class:active={$page.url.pathname === `${base}${item.to}`}
				>
	...
	.nav-menu {
		...
		&-item {
			...
			&.active {
				...
				color: var(--main) !important// Set the desired color for the active link;
			}
		}
	}
</style>
